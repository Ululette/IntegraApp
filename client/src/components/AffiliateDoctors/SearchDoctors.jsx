...
/*     useEffect(() => {
    ...
    useEffect(() => {
        const fetchSpeciality = async () => {
            let { data: speciality } = await supabase.from('medical_specialities').select('*');
    ...
    useEffect(() => {
        const fetchDoctors = async () => {
            let { data: doctors } = await supabase.from('medics').select('name, lastname, email, phone_number, profilePic, address, medical_specialities(id, name)');
            setDoctors(doctors);
            console.log(doctors)
        };
        fetchDoctors();
...
const changePage = ({ selected }) => {
  ...
  return (
   ...
   {/*      <NativeSelect
        ...
        <option className="labels">Especialidad</option>
        {speciality.map((x) => (
              <option value={x.name} key={x.id}>
                {x.name}
              </option>
  ...
  .map(d => <button onClick= {togglePopup}> {`${d.name} ${d.lastname}`}</button>)}</div>
