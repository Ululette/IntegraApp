...
export default function AcccessibleTable({ plans }) {
  ...
  async function getBenefits() {
    ...
    let benef = benefits.filter(benefit => benefit.description !== 'Básico')
    setCompBenef(benef);
  }
  ...
  return (
    <TableContainer style={{ background: "#f1f1f1", }} component={Paper}>
      {planNames.slice(1).length && <Table className={classes.table} aria-label="caption table">
        <TableHead style={{ background: "#d7f7f5" }}>
          <TableRow>
            <TableCell style={{ width: 400 }}>BENEFICIOS EXTRA DE PLANES SUPERIORES</TableCell>
            {planNames.slice(1).map((plan, index) => (
              <TableCell key={index} align="center">INTEGRA {plan}</TableCell>
            ))}
          </TableRow>
        </TableHead>
        <TableBody>
          {/* Crea la fila a partir del arreglo */}
          {compBenef.length && compBenef.map((benef, index1) => (
            <TableRow key={index1}>
              <TableCell key={index1} component="th" scope="row">{benef.title}</TableCell>
              ...
              {planNames.slice(1).map((plan, index2) => (
                <TableCell key={index2} align="center">
                  <Checkbox
                    key={index2}
                    style={{
                      color: "#41aea9", height: 3
                    }}
                    edge="start"
                    checked={parseInt(benef.description.slice(11, 14)) <= parseInt(plan)}
                    tabIndex={-1}
                    disableRipple
                  />
                </TableCell>
          ...
          {/* Fila con importes  */}
          <TableRow >
            <TableCell style={{ background: "#d7f7f5" }} component="th" scope="row">Importe:</TableCell>
