...
function MyPlan({ affiliateData }) {
  ...
  return (
    <div className={styles.container}>
      {(plan && !comp) && <>
        <header className={`${styles.header} ${styles[`integra${plan.name.substring(8)}`]}`}>
          <h3>Mi Plan</h3>
          <h2>{plan.name}</h2>
        </header>
        ...
        <button className={styles.buttonCompare} onClick={handleCompare}>
          <h3>Otras opciones</h3>
        </button>
      </>}
      {(plan && comp) && <>
        <button className={styles.buttonBack} onClick={handleCompare} >
          <p className={styles.nuni}>Volver</p>
        </button>
        {/* <hr className={styles.divider} /> */}
        <CompareMyPlan  plandata={plan} familymembers={numberFamiliars}/>
      </>}
    </div>
  );
}
