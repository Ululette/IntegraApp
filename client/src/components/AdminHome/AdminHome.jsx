...
function AdminHome({ firebase }) {
  ...
  const handleOpenModal = (plan) => {
    setModalPlan({
      ...
      price: plan.price,
      benefits: plan.benefits.map((benefit) => benefit.benefit_description),
    });
    setOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    const { data, error } = await supabase
      .from("plans")
      .update({ description: e.target[0].value, price: e.target[1].value })
    ...
    <div className={styles.container}>
      ...
      <section className={styles.plansContainer}>
        {allPlans.map((plan, index) => (
          <div key={`plan-${index}`} className={styles.detailContainer}>
            ...
            <ul className={styles.benefitContainer}>
              {plan.benefits.map((el, i) => (
                <li key={`benefit-${index}-${i}`}>{el.benefit_description}</li>
              ))}
            </ul>
            ...
            <DialogContent>
              ...
              <Box m={2}>
                <Autocomplete
                  ...
                  options={allBenefits}
                  getOptionLabel={(option) => option.benefit_description}
                  ...
                  defaultValue={allBenefits.filter((benefit) =>
                    modalPlan.benefits.includes(benefit.benefit_description)
                  )}
